...
8. **No brand color/type/spacing replacement has been applied anywhere in this package.** That is the explicit scope of the *next* pass. Autopay Design System 2.0 tokens are now available (see [`DS2-HARMONIZATION.md`](./DS2-HARMONIZATION.md) for the first real comparison) — typography and brand accent colors already match; the neutral/semantic color scale is the one confirmed, unresolved gap. Spacing has no DS2 source to compare against at all.
9. **`"use client"` is applied per built bundle, not per component** (see `AUDIT.md` section 8) — importing anything from `/organisms`, even a fully static one, pulls in a client bundle, because `tsup` bundles the whole layer into one file. Real per-component RSC boundaries would need a build restructure.
