...
## 16. Storybook homepage + autodocs; a redundant Vercel deploy caught and reverted (2026-09-14)

Requested: sidebar navigation with search and categories, a homepage, and an auto-updating hosted site. The sidebar/search/categories were already native Storybook features requiring no work. Added:
- `@storybook/addon-docs` (devDependency, pinned to the exact installed Storybook version, `10.6.0`) — enables the "Docs" tab/autodocs per component and MDX page support. Added to `.storybook/main.ts`'s `addons` array and `stories` glob (`*.mdx`); `tags: ["autodocs"]` added to `.storybook/preview.tsx`.
- `src/Introduction.mdx` — a real homepage (what this package is, where things are, quick usage, the `CookieConsentScript` "always include" callout, DS2 harmonization status table) — pinned first via `parameters.options.storySort` in `preview.tsx`, so it's what loads by default.
