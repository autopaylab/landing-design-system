...
**2026-09-11 addendum:** the flat `mt-32` inter-section spacing (no mobile/desktop split) called out above turned out to be a real, reported problem — confirmed too large on mobile on the live site. Fixed to `mt-16 md:mt-32` across every organism using it; see `AUDIT.md` §13. This doesn't change the conclusion above (still no DS2 spacing source to harmonize *against*), it just gives the existing baseline a mobile/desktop split, matching the convention a few organisms already used (`py-14 md:py-20`).
